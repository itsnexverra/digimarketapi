...
// Helper to generate reliable image URLs across distinct render seeds for batch generation
function generateFallbackImageUrls(prompt, count = 1) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const promptVariation = count > 1 ? `${cleanPrompt}, render variation ${i + 1} masterpiece 8k` : cleanPrompt;
    const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(promptVariation)}?width=1024&height=1024&nologo=true&seed=${seed}&model=${model}`;
    urls.push(url);
  }
  return urls;
}
...
app.post("/api/v1/generate", async (req, res) => {
  try {
    ...
    const imageUrls = generateFallbackImageUrls(prompt, numOutputs);
      ...
      try {
        ...
        if (api.category === "Image" || api.type === "Modal" || (api.name && api.name.toLowerCase().includes("image"))) {
          if (mediaUrls.length >= requestedNum) {
            ...
            const extra = generateFallbackImageUrls(prompt, needed);
            const combined = [...mediaUrls, ...extra];
            result = { type: "image", url: combined[0], urls: combined };
          } else {
            const fallbackUrls = generateFallbackImageUrls(prompt || "beautiful artwork", requestedNum);
