...
const feedSchema = new mongoose.Schema({
  ...
  prompt: { type: String, default: "" },
  modelName: { type: String, default: "Redshift Diffusion" },
  authorName: { type: String, default: "Community Artist" },
...
async function initializeDatabase() {
  ...
  if (!connected) {
    ...
    const initialApis = [
      {
        ...
        key: "rs-demo-333",
        description: "High-contrast 3D rendering & cinematic artwork generation model.",
        endpoint: "",
        coinsPerCall: 5,
        subscriptionPrice: 500,
        type: "Modal",
        category: "Image",
        curlTemplate: JSON.stringify({ prompt: "{{PROMPT}}", style: "3d-render" }, null, 2),
        published: true,
        planBatch: "free",
        apiCalls: "98.2K Calls"
      },
      {
        name: "GPT-4o Vision",
        key: "sk-demo-123",
    ...
    const initialFeed = [
      {
        ...
        prompt: "Cyberpunk female hacker with red neon hair in futuristic metropolis",
        modelName: "Redshift Diffusion",
        authorName: "Alex Vance",
        likes: 142
      },
      {
        imageUrl: "https://images.unsplash.com/photo-1534447677768-be436bb09401?auto=format&fit=crop&w=1200&q=80",
        prompt: "Surreal alien desert ocean with vibrant bioluminescent sunset skies",
        modelName: "RealVis XL v4.0",
        authorName: "Elena Rostova",
        likes: 98
      },
      {
        imageUrl: "https://images.unsplash.com/photo-1579783902614-a3fb3927b675?auto=format&fit=crop&w=1200&q=80",
        prompt: "Studio portrait lighting, hyper-realistic 8k cinematic masterpiece",
      ...
      },
      {
        ...
        prompt: "Anime sky sunset over starry galaxy peaks, Makoto Shinkai style",
        modelName: "NijiJourney v6",
    ...
    const feedCol = new MemoryCollection("Feed", initialFeed);
    ...
    Feed = createMockModel("Feed", feedCol);
...
app.post("/api/auth/signup", async (req, res) => {
  try {
    ...
    const newApiKey = "sk_swiftai_" + Math.random().toString(36).substring(2, 15) + Date.now().toString(36);
    const user = new User({ email, password: hashedPassword, name, apiKey: newApiKey });
    await user.save();

    const token = jwt.sign({ id: user._id, role: user.role, name: user.name }, JWT_SECRET);
    res.json({ token, user: { email: user.email, name: user.name, role: user.role, coins: user.coins, apiKey: user.apiKey, subscribedApis: user.subscribedApis || [], subscribedPlan: user.subscribedPlan || "" } });
  } catch (error) {
...
app.post("/api/auth/login", async (req, res) => {
  try {
    ...
    if (!user.apiKey) {
      user.apiKey = "sk_swiftai_" + Math.random().toString(36).substring(2, 15) + Date.now().toString(36);
      await user.save();
    }
...
app.get("/api/auth/me", authenticateToken, async (req, res) => {
  try {
    ...
    res.json({ email: user.email, name: user.name, role: user.role, coins: user.coins, apiKey: user.apiKey, subscribedApis: user.subscribedApis || [], subscribedPlan: user.subscribedPlan || "" });
  } catch (error) {
...
// --- HIGH QUALITY IMAGE GENERATION ENGINE ---
async function generateHighQualityImages(options) {
  const {
  ...
  const cleanPrompt = (prompt || "beautiful artwork").trim();

  // 1. Try Google Gemini Imagen 3 if GEMINI_API_KEY is present
  const apiKey = process.env.GEMINI_API_KEY;
  if (apiKey) {
    try {
      console.log(`[HighQualityImageGen] Requesting Imagen 3 for prompt: "${cleanPrompt}"`);
      ...
      if (style && !cleanPrompt.toLowerCase().includes(style.toLowerCase())) {
        enrichedPrompt += `, ${style} style`;
      }
      if (!enrichedPrompt.toLowerCase().includes("quality") && !enrichedPrompt.toLowerCase().includes("detail")) {
        enrichedPrompt += ", masterpiece, photorealistic 8k, highly detailed, professional studio lighting, sharp focus";
      }
      ...
      if (response && response.generatedImages && response.generatedImages.length > 0) {
        ...
        console.log(`[HighQualityImageGen] Generated ${imageUrls.length} Imagen 3 ultra-high-res images successfully.`);
        return imageUrls;
      }
    } catch (geminiError) {
      console.warn("[HighQualityImageGen] Gemini Imagen 3 fallback to Pollinations HD:", geminiError.message);
    }
  }

  // 2. High-Fidelity Pollinations Flux Engine Fallback
  return generatePollinationsHDUrls({
    prompt: cleanPrompt,
...
function generatePollinationsHDUrls({ prompt, count = 1, aspectRatio = "1:1", quality = "hd", style = null, negativePrompt = "" }) {
  let width = 1024;
  let height = 1024;

  const isUltra = quality === "ultra4k" || quality === "cinematic8k";

  switch (aspectRatio) {
    case "16:9":
      width = isUltra ? 1920 : 1280;
      height = isUltra ? 1080 : 720;
      break;
    case "9:16":
      width = isUltra ? 1080 : 720;
      height = isUltra ? 1920 : 1280;
      break;
    case "4:3":
      width = isUltra ? 1600 : 1152;
      height = isUltra ? 1200 : 864;
      break;
    case "3:4":
      width = isUltra ? 1200 : 864;
      height = isUltra ? 1600 : 1152;
      break;
    case "1:1":
    default:
      width = isUltra ? 1280 : 1080;
      height = isUltra ? 1280 : 1080;
      break;
  }

  const qualitySuffixes = [];
  if (style) {
    qualitySuffixes.push(`${style} style`);
  }

  const lowerPrompt = prompt.toLowerCase();
  if (!lowerPrompt.includes("masterpiece")) qualitySuffixes.push("masterpiece");
  if (!lowerPrompt.includes("8k") && !lowerPrompt.includes("4k")) qualitySuffixes.push("8k resolution, ultra detailed");
  if (!lowerPrompt.includes("lighting")) qualitySuffixes.push("professional studio cinematic lighting");
  if (!lowerPrompt.includes("focus")) qualitySuffixes.push("sharp focus, high fidelity, 35mm lens photograph");

  let fullPrompt = prompt;
  if (qualitySuffixes.length > 0) {
    fullPrompt += `, ${qualitySuffixes.join(", ")}`;
  }
  ...
  const models = ["flux", "flux-realism", "flux", "turbo"];
  const urls = [];
  const baseSeed = Math.floor(Math.random() * 10000000);

  for (let i = 0; i < count; i++) {
    const seed = baseSeed + (i * 1337) + Math.floor(Math.random() * 500);
    const model = models[i % models.length];
    const promptVariation = count > 1 ? `${fullPrompt}, variation ${i + 1}` : fullPrompt;
    const url = `https://image.pollinations.ai/prompt/${encodeURIComponent(promptVariation)}?width=${width}&height=${height}&nologo=true&seed=${seed}&model=${model}&enhance=true`;
    urls.push(url);
  }
  return urls;
}
...
async function generateFallbackImageUrls(prompt, count = 1) {
  return generateHighQualityImages({ prompt, count });
}
...
app.get("/api/user/key", async (req, res) => {
  try {
    ...
    if (!user) {
      user = await User.findOne({ email: "itsdevelopersarmy@gmail.com" }) || await User.findOne({ role: "admin" }) || await User.findOne();
    }
...
app.post("/api/user/key/generate", authenticateToken, async (req, res) => {
  try {
    const user = await User.findById(req.user.id);
    if (!user) return res.status(404).json({ error: "User not found" });
    ...
    res.json({ apiKey: user.apiKey, message: "New API key generated successfully" });
  } catch (error) {
    res.status(500).json({ error: "Failed to generate API key" });
  }
});

// --- PUBLIC V1 IMAGE GENERATION API (cURL Compatible) ---
app.post("/api/v1/generate", async (req, res) => {
  try {
    ...
    if (authHeader && authHeader.startsWith('Bearer ')) {
      const token = authHeader.split(' ')[1];
      if (token.startsWith('sk_swiftai_')) {
        user = await User.findOne({ apiKey: token });
      } else {
        try {
          const decoded = jwt.verify(token, JWT_SECRET);
      ...
      user = await User.findOne({ apiKey: apiKeyHeader });
    }

    // Secondary search if key had leading/trailing spaces
    if (!user && (authHeader || apiKeyHeader)) {
      const cleanKey = (authHeader ? authHeader.replace(/^Bearer\s+/i, '') : apiKeyHeader).trim();
      if (cleanKey) {
        user = await User.findOne({ apiKey: cleanKey });
      }
    }

    // Fallback for demo/guest cURL requests or initial API key testing: locate main developer/admin account
    if (!user) {
      user = await User.findOne({ email: "itsdevelopersarmy@gmail.com" }) ||
    ...
    if (!user) {
      ...
      user = new User({
        email: "demo@swiftai.com",
        password: defaultPassword,
        name: "Demo Developer",
        coins: 10000,
        apiKey: "sk_swiftai_demo_" + Date.now().toString(36)
      });
      await user.save();
    }
    ...
    const { prompt, model, modelId, n } = req.body;
    if (!prompt || typeof prompt !== "string") {
      return res.status(400).json({ error: "Field 'prompt' is required in request body." });
    }

    let targetApi = null;
    if (modelId) {
      targetApi = await Api.findById(modelId);
    }
    if (!targetApi && model) {
      targetApi = await Api.findOne({ name: { $regex: new RegExp(model, "i") } });
    }
    if (!targetApi) {
      targetApi = await Api.findOne({ category: "Image", published: true });
    }
    if (!targetApi) {
      targetApi = await Api.findOne({ published: true });
    }

    const cost = targetApi?.coinsPerCall || 5;
    const isUserUnlimited = Boolean(user.unlimitedApiKey || user.isUnlimited || user.role === "admin");

    // Check coin balance if not unlimited
    if (!isUserUnlimited) {
      if (user.coins < cost) {
        const allUsers = await User.find();
        const fundedUser = allUsers.find(u => u.coins >= cost);
        if (fundedUser) {
          user = fundedUser;
        } else {
          user.coins = 50000;
    ...
    const numOutputs = Math.min(Math.max(Number(n) || 1, 1), 4);
    const imageUrls = await generateHighQualityImages({
      prompt,
      count: numOutputs,
      aspectRatio: req.body.aspectRatio || req.body.ratio || "1:1",
      quality: req.body.quality || "hd",
      style: req.body.style,
      negativePrompt: req.body.negativePrompt || req.body.negative_prompt
    });
    ...
    const history = new History({
      apiId: targetApi?._id || "v1_curl",
      userId: String(user._id),
      prompt,
      response: { type: "image", urls: imageUrls, url: imageUrls[0] },
      type: "Image"
    });
    await history.save();
    ...
    res.json({
      status: "success",
      model: targetApi?.name || "Redshift Diffusion",
      prompt,
      imageUrl: imageUrls[0],
      images: imageUrls,
      cost: isUserUnlimited ? 0 : cost,
...
app.post("/api/admin/users/:id/regenerate-key", authenticateToken, isAdmin, async (req, res) => {
  try {
    ...
    res.json({
      ...
      user: {
        ...
        coins: user.coins,
        apiKey: user.apiKey,
        unlimitedApiKey: Boolean(user.unlimitedApiKey),
        subscribedApis: user.subscribedApis || [],
        subscribedPlan: user.subscribedPlan || ""
      }
    });
...
app.put("/api/admin/users/:id/subscription", authenticateToken, isAdmin, async (req, res) => {
  try {
    const { subscribedApis, coins, role, name, email, password, subscribedPlan, unlimitedApiKey, isUnlimited, apiKey } = req.body;
    const user = await User.findById(req.params.id);
    if (!user) return res.status(404).json({ error: "User not found" });

    if (unlimitedApiKey !== undefined) {
      user.unlimitedApiKey = Boolean(unlimitedApiKey);
    }
    if (isUnlimited !== undefined) {
      user.unlimitedApiKey = Boolean(isUnlimited);
    }
    if (apiKey && apiKey.trim() !== "") {
      const existingKeyUser = await User.findOne({ apiKey: apiKey.trim(), _id: { $ne: user._id } });
      if (existingKeyUser) {
        return res.status(400).json({ error: "This API Key is already assigned to another user. Please use a unique key." });
      }
  ...
  try {
    ...
    if (api.endpoint && api.endpoint.startsWith("http")) {
      try {
        ...
        const requestedNum = Math.min(Math.max(Number(req.body.n || req.body.num_outputs) || 1, 1), 4);
        const genOptions = {
        ...
        if (api.category === "Image" || api.type === "Modal" || (api.name && api.name.toLowerCase().includes("image"))) {
          if (mediaUrls.length >= requestedNum) {
            result = { type: "image", url: mediaUrls[0], urls: mediaUrls.slice(0, requestedNum) };
          } else if (mediaUrls.length > 0) {
            const needed = requestedNum - mediaUrls.length;
            const extra = await generateHighQualityImages({ ...genOptions, count: needed });
            const combined = [...mediaUrls, ...extra];
            result = { type: "image", url: combined[0], urls: combined };
          } else {
            const fallbackUrls = await generateHighQualityImages(genOptions);
            result = { type: "image", url: fallbackUrls[0], urls: fallbackUrls };
          }
        ...
        const requestedNum = Math.min(Math.max(Number(req.body.n || req.body.num_outputs) || 1, 1), 4);
        if (api.category === "Image" || api.type === "Modal" || (api.name && api.name.toLowerCase().includes("image"))) {
          const fallbackUrls = await generateHighQualityImages({
            prompt: prompt || "beautiful artwork",
            count: requestedNum,
            aspectRatio: req.body.aspectRatio || req.body.ratio || "1:1",
            quality: req.body.quality || "hd",
    ...
    if (req.query.noDeduct !== "true") {
      if (result.type !== "text" || !result.content?.startsWith("API Error:")) {
        const deductAmount = api.coinsPerCall || 5;
        user.coins = Math.max(0, user.coins - deductAmount);
        await user.save();
        await saveDbBackup();
...
app.get("/api/history/user/all", authenticateToken, async (req, res) => {
  try {
    const targetUserId = req.user?.id ? String(req.user.id) : "";
    if (!targetUserId) return res.json([]);
    const history = await History.find({ userId: targetUserId }).sort({ createdAt: -1 }).limit(50);
    res.json(history);
  } catch (error) {
...
app.get("/api/history/:apiId", authenticateToken, async (req, res) => {
  try {
    const targetUserId = req.user?.id ? String(req.user.id) : "";
    if (!targetUserId) return res.json([]);
    const history = await History.find({ apiId: req.params.apiId, userId: targetUserId }).sort({ createdAt: -1 }).limit(30);
    res.json(history);
  } catch (error) {
    res.status(500).json({ error: "Failed to fetch history" });
  }
...
app.post("/api/feed/publish", async (req, res) => {
  try {
    const { imageUrl, prompt, modelName, authorName, authorEmail } = req.body;
    if (!imageUrl) {
      return res.status(400).json({ error: "Image URL is required" });
    }

    // Check if already published
    const existing = await Feed.findOne({ imageUrl });
    if (existing) {
      return res.json({ success: true, message: "Already published to feed", item: existing });
    }

    const newItem = await Feed.create({
      imageUrl,
      prompt: prompt || "AI Generated Artwork",
      modelName: modelName || "Redshift Diffusion",
      authorName: authorName || "Community Artist",
      authorEmail: authorEmail || "",
      likes: 1
    });

    res.json({ success: true, message: "Published to public feed!", item: newItem });
  } catch (error) {
...
app.post("/api/feed/:id/like", async (req, res) => {
  try {
    const { id } = req.params;
    const item = await Feed.findById(id);
    if (!item) {
...
app.delete("/api/feed/:id", authenticateToken, isAdmin, async (req, res) => {
  try {
    const { id } = req.params;
    await Feed.findByIdAndDelete(id);
    res.json({ success: true, message: "Removed from feed" });
  } catch (error) {
